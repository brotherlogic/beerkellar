---
description: Gather requirements and formulate a Product Requirements Document (PRD)
---

# 🏷️ The `beerkellar-needs-requirements` Label Workflow

When a GitHub issue is labeled with `beerkellar-needs-requirements` (or its variant `beerkellar-need-requirements`), the AI assistant (**Beerkellar**) is triggered to run a requirements-gathering process. This stage focuses strictly on **what** needs to be built and **why**, avoiding any early technical implementation details.

## 🔄 Workflow Lifecycle

```mermaid
graph TD
    A[Issue Labeled beerkellar-needs-requirements] --> B[1. Request Brief Description]
...
2. **Remove the Label:** Remove the `beerkellar-needs-requirements` (or `beerkellar-need-requirements`) label from the parent issue to signify completion of the requirements phase.
3. **Create Sub-Issue:** Programmatically create a GitHub sub-issue to track the subsequent step:
   ...
   - **Sub-Issue Label:** `beerkellar-needs-implementation-plan`
